...
use crate::transformer::{get_output_dimensions, transform_rgba};
...
fn update(_nano_time: i64) {
    ...
    unsafe {
        ...
        let (out_w, out_h) = get_output_dimensions(width, height);
        ...
        let (out_w, out_h, out) = transform_rgba(sprite, width, height, transform);
